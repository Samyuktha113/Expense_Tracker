...
public function getExpenseById($ID){
  $stmt = $this->pdo->prepare("SELECT * FROM expense WHERE ID = :id");
  $stmt->bindParam(":id", $ID, PDO::PARAM_INT);
  $stmt->execute();
...
public function updateExpense($ID, $Item, $Cost, $Date){
  $stmt = $this->pdo->prepare("UPDATE expense SET Item = :item, Cost = :cost, Date = :date WHERE ID = :id");
  $stmt->bindParam(":id", $ID, PDO::PARAM_INT);
